    ...
    for i in range(num_frames):
        with open(json_files[i]) as f:
            ...
            try:
                ...
                print("keypoints array size = ", i, len(keypoints))
...
def _bicep_curl(pose_seq):
    ...
    if upper_arm_torso_range > 35.0:
        correct = False
        feedback += '\n\u2022 Your upper arm shows significant rotation around the shoulder when curling. Try holding your upper arm still, parallel to your chest, ' + \
...
def _squat(pose_seq):
    ...
    if upper_leg_torso_range < 100.0:
        correct = False
        feedback += '\n\u2022 You are bending your back too much, your back and thighs should be approximately perpendicular to each other.\n'
    
    if upper_leg_lower_leg_min > 75.0:
        correct = False
        feedback += '\n\u2022 You are not bending your knees enough, Make sure the thighs are parallel to the ground.\n'

    if correct:
        return (correct, '\n\u2022 Exercise performed correctly! Thighs were parallel to the ground, knees were bent correctly.')
    else:
        return (correct, feedback)
